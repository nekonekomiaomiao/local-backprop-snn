...
if not __import__("os").path.exists(CSV):
    with open(CSV, "w", encoding="utf-8") as fh:
        fh.write("which,seed,N,SAMPLE_T,R,alpha,FDA,BIAS,TARGET,KAPPA,GAMMA,DT,RESET,ISI,TAU_M,TAU_E,"
                 "loss_plateau,loss_std,frozen_acc,"
...
# usage: python mnist_lif_table.py <shared> <N> <SAMPLE_T> <R> <alpha> <FDA> <BIAS> <TARGET> [seed] [KAPPA] [GAMMA] [DT] [ISI] [TAU_M] [TAU_E] [RESET]
which = sys.argv[1]
...
ISI_STEPS = int(sys.argv[13]) if len(sys.argv) > 13 else 0
TAU_M = float(sys.argv[14]) if len(sys.argv) > 14 else 0.0
TAU_E = float(sys.argv[15]) if len(sys.argv) > 15 else 0.2
RESET_PER_SAMPLE = int(sys.argv[16]) if len(sys.argv) > 16 else 0
FINAL_EVAL_N = 500
...
if which == "shared":
    sys.argv = ["mnist_shared.py", str(SEED), str(N_TRAIN), str(SAMPLE_T), str(R_IN), str(ALPHA),
                ...
                str(RESET_PER_SAMPLE), str(ISI_STEPS), str(TAU_M), str(TAU_E)]
    import mnist_shared as m
else:
    raise SystemExit("mnist_lif_table only supports shared for LIF+ISI protocol")
...
# --- trajectory (rolling window, dumped every 200 samples) ---
TRAJ_FILE = f"traj_T{int(TARGET)}_K{KAPPA}_TM{TAU_M}_ISI{ISI_STEPS}_TE{TAU_E}_RS{RESET_PER_SAMPLE}.csv"
S_win = np.zeros((3, 5))   # windowed covariance accumulators (200-sample window)
win_align = [[], [], []]   # windowed per-sample descent alignment per layer
WIN = 200
...
for smp in range(N_TRAIN):
    ...
    for st in range(m.steps):
        ...
        for gi in range(3):
            e = Eb[gi]
            d = ds[gi]
            S_win[gi, 0] += e.sum(); S_win[gi, 1] += d.sum()
            S_win[gi, 2] += (e * d).sum(); S_win[gi, 3] += (e * e).sum(); S_win[gi, 4] += (d * d).sum()
            if smp >= cov_start:
    ...
    for li, (_, sl) in enumerate(LAYERS):
        ...
        if gn > 1e-9 and dn > 1e-9:
            ...
            n_cos[li, 0] += c
            n_cos[li, 1] += c * c
        win_align[li].append(-c if (gn > 1e-9 and dn > 1e-9) else float("nan"))
        if len(win_align[li]) > WIN:
            win_align[li].pop(0)
    ...
    if (smp + 1) % WIN == 0:
        ...
        # --- trajectory dump (rolling 200-sample window) ---
        n_el_w = [WIN * m.steps * (len(m.E1) if gi == 0 else (len(m.PRE2) if gi == 1 else len(m.PRE3))) for gi in range(3)]
        corr_w = []
        bias_w = []
        for gi in range(3):
            me = S_win[gi, 0] / n_el_w[gi]
            md = S_win[gi, 1] / n_el_w[gi]
            cov = S_win[gi, 2] / n_el_w[gi] - me * md
            vare = S_win[gi, 3] / n_el_w[gi] - me * me
            vard = S_win[gi, 4] / n_el_w[gi] - md * md
            corr_w.append(cov / np.sqrt(vare * vard + 1e-12))
            bias_w.append(abs(cov) / (abs(me * md) + 1e-12))
        a_mean = [float(np.nanmean(w)) if any(~np.isnan(w)) else float("nan") for w in win_align]
        a_std = [float(np.nanstd(w)) if any(~np.isnan(w)) else float("nan") for w in win_align]
        a_all = float(np.nanmean([x for w in win_align for x in w if not np.isnan(x)])) if any(not np.isnan(x) for w in win_align for x in w) else float("nan")
        a_all_std = float(np.nanstd([x for w in win_align for x in w if not np.isnan(x)])) if any(not np.isnan(x) for w in win_align for x in w) else float("nan")
        line = (f"{smp + 1},{np.mean(losses[max(0, smp - 99):smp + 1]):.5f},{np.mean(accs[max(0, smp - 99):smp + 1]):.3f},"
                f"{a_mean[0]:+.4f},{a_mean[1]:+.4f},{a_mean[2]:+.4f},{a_all:+.4f},{a_all_std:.4f},"
                f"{corr_w[0]:+.4f},{corr_w[1]:+.4f},{corr_w[2]:+.4f},"
                f"{bias_w[0]:.3f},{bias_w[1]:.3f},{bias_w[2]:.3f}")
        if not os.path.exists(TRAJ_FILE):
            with open(TRAJ_FILE, "w", encoding="utf-8") as fh:
                fh.write("step,loss_roll,acc_roll,alW1,alW2,alW3,alAll,alAll_std,cW1,cW2,cW3,bW1,bW2,bW3\n")
        with open(TRAJ_FILE, "a", encoding="utf-8") as fh:
            fh.write(line + "\n")
        S_win[:] = 0
        win_align[:] = [[], [], []]
...
for ii in idx:
    ...
    if m.ISI_STEPS > 0 and not RESET_PER_SAMPLE:
        zx = np.zeros(784)
        zy = np.zeros(m.NOUT)
        for _ in range(m.ISI_STEPS):
            m.spiking_step(zx, zy, learn=False)
...
row = (f"{which},{SEED},{N_TRAIN},{SAMPLE_T},{R_IN},{ALPHA},{FDA},{BIAS},{TARGET},{KAPPA},{GAMMA},{DT},"
       f"{RESET_PER_SAMPLE},{ISI_STEPS},{TAU_M},{TAU_E},"
...
with open("mnist_lif_table_results.csv", "a", encoding="utf-8") as fh:
    fh.write(row + "\n")