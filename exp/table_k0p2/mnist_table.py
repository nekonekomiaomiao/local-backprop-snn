...
if not __import__("os").path.exists(CSV):
    with open(CSV, "w", encoding="utf-8") as fh:
        fh.write("which,seed,N,SAMPLE_T,R,alpha,FDA,BIAS,TARGET,loss_plateau,loss_std,frozen_acc,"
...
# usage: python mnist_table.py <shared|stride> <N> <SAMPLE_T> <R> <alpha> <FDA> <BIAS> <TARGET> [seed] [KAPPA] [GAMMA]
which = sys.argv[1]
...
if which == "shared":
    sys.argv = ["mnist_shared.py", str(SEED), str(N_TRAIN), str(SAMPLE_T), str(R_IN), str(ALPHA),
                str(FDA), str(BIAS), str(TARGET), str(FINAL_EVAL_N), "0.02", str(KAPPA), str(GAMMA)]
...
row = (f"{which},{SEED},{N_TRAIN},{SAMPLE_T},{R_IN},{ALPHA},{FDA},{BIAS},{TARGET},"
